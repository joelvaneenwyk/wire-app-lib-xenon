...
public interface SessionsDAO {
    @SqlUpdate(
        "INSERT INTO Sessions (id, sid, data) VALUES (:id, :sid, :data) ON CONFLICT (id, sid) DO UPDATE SET data = EXCLUDED.data"
    )
    int insert(@Bind("id") String id, @Bind("sid") String sid, @Bind("data") byte[] data);

    @SqlQuery("SELECT * FROM Sessions WHERE id = :id AND sid = :sid FOR UPDATE")
    @RegisterColumnMapper(_Mapper.class)
    Session get(@Bind("id") String id, @Bind("sid") String sid);
